      ...
      <div className="overflow-hidden rounded-3xl border border-(--color-border) bg-(--color-surface)">
        ...
        {!isLoading && !isError && data.length > 0 && (
          <>
            ...
            {/* Pagination */}
            {totalPages > 1 && (
              <div className="flex items-center justify-center gap-3 border-t border-(--color-border) px-4 py-3 sm:px-6">
                <button
                  type="button"
                  onClick={handlePrevious}
                  disabled={page === 1 || isFetching}
                  className="flex items-center gap-1.5 rounded-xl border border-(--color-border-secondary) px-3 py-2 text-xs font-medium text-(--color-text-secondary) transition hover:border-(--color-primary) hover:text-(--color-primary-light) disabled:cursor-not-allowed disabled:opacity-40"
                >
                  <ChevronLeft size={15} />
                  Previous
                </button>

                <span className="rounded-xl border border-(--color-border-secondary) bg-(--color-surface-hover) px-3 py-2 text-xs font-medium text-(--color-text-muted)">
                  {page} / {totalPages}
                </span>
